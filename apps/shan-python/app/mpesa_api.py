...
class MpesaAPI:
    ...
    def init_app(self, app):
        ...
        if not all([self.consumer_key, self.consumer_secret, self.shortcode, self.passkey, self.callback_url]):
            app.logger.error("One or more M-Pesa credentials are missing in config")
    ...
    @property
    def api_url(self):
        return self.sandbox_url if self.app.config.get('MPESA_ENVIRONMENT') == 'sandbox' else self.base_url
    ...
    def refresh_token(self):
        try:
            ...
            self.app.logger.info(f"Token refresh response status: {response.status_code}")

            if response.status_code == 200:
                token_data = response.json()
                self.set_access_token(token_data['access_token'], token_data.get('expires_in', 3599))
                self.app.logger.info("M-Pesa access token refreshed successfully")
                return True
            else:
                self.app.logger.error(f"Token refresh failed: {response.text}")
                return False

        except Exception as e:
            self.app.logger.error(f"Exception while refreshing token: {e}", exc_info=True)
            return False

    def ensure_valid_token(self):
        if not self.is_token_valid():
            self.app.logger.info("Token expired or missing, refreshing...")
    ...
    def _format_phone_number(self, phone_number):
        ...
        if not phone_number:
            self.app.logger.warning("No phone number provided to _format_phone_number")
            return ""
        ...
        if phone_number.startswith('0'):
            ...
            self.app.logger.warning(f"Unexpected phone number format: {phone_number}")
            return phone_number
    ...
    def verify_transaction(self, transaction_id):
        if not self.ensure_valid_token():
            self.app.logger.error("Failed to get valid access token for M-Pesa API")
            return False
        ...
        try:
            ...
            self.app.logger.info(f"Transaction query status: {response.status_code}")

            if response.status_code == 200:
                result = response.json()
                self.app.logger.info(f"Transaction query result: {result}")
                return result.get('ResultCode') == "0"

            self.app.logger.error(f"Transaction query failed: {response.text}")
            return False

        except Exception as e:
            self.app.logger.error(f"Error during transaction verification: {e}", exc_info=True)
            return False
    ...
    def initiate_stk_push(self, phone_number, amount, account_reference, transaction_description=None):
        ...
        try:
            ...
            self.app.logger.info(f"STK Push status: {response.status_code}")

            if response.status_code == 200:
                result = response.json()
                self.app.logger.info(f"STK Push response: {result}")
                return result.get('CheckoutRequestID') if result.get('ResponseCode') == "0" else None

            self.app.logger.error(f"STK Push failed: {response.text}")
            return None

        except Exception as e:
            self.app.logger.error(f"Error initiating STK push: {e}", exc_info=True)
            return None
